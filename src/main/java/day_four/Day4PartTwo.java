...
public class Day4PartTwo {
     ...
     *
     * @return An integer representing the sum of the remaining numbers of the worst BINGO board * the last number called out
     */
    public int calculateWorstBoard() {
        // Read the file
        ...
        // Determine the worst board
        return playBingo(allBoardRows, allBoardColumns, numbers);
    }

    /**
     * Simulates a game of BINGO and selects the worst board from the group of all boards
     *
     ...
     * @return An integer representing the sum of the remaining numbers of the worst BINGO board * the last number called out. An invalid integer -1 if no worst board.
     */
    private int playBingo(ArrayList<HashSet<Integer>[]> allBoardRows, ArrayList<HashSet<Integer>[]> allBoardColumns, ArrayList<Integer> numbers) {
        ...
        for (int number: numbers) {
            ...
            // Iterate by boards
            int board = 0;
            while (board < allBoardRows.size()) {
                boolean removedBoard = false;

                // Iterate by rows and columns
                int index = 0;
                while (index < allBoardRows.get(board).length) {
                    // Remove number from row and column
                    allBoardRows.get(board)[index].remove(number);
                    allBoardColumns.get(board)[index].remove(number);

                    // Check if rows meet criteria for worst board
                    if (allBoardRows.get(board)[index].isEmpty() && allBoardRows.size() == 1) {
                        return number * calculateBoardScore(allBoardRows.get(board));
                    // Check if cols meet criteria for worst board
                    } else if (allBoardColumns.get(board)[index].isEmpty() &&  allBoardColumns.size() == 1) {
                        return number * calculateBoardScore(allBoardColumns.get(board));
                    }

                    // Remove non-worst boards
                    if (allBoardRows.get(board)[index].isEmpty() || allBoardColumns.get(board)[index].isEmpty()) {
                        removedBoard = true;
                        allBoardRows.remove(board);
                        allBoardColumns.remove(board);
                        break;
                    }
                    index += 1;
                }
                // Ensure that iteration only occurs as needed
                if (!removedBoard) {
                    board += 1;
                }
            }
